...
{
    public partial class DisplayMaster : Form,ToolStripAccess
    {
        ...
        private static DisplayMaster _instance;
        ...
        private void DisplayMaster_Load(object sender, EventArgs e)
        {
            News();         
            txtsearch.Select();
        }
        ...
        public void Saves()
        {
            ...
            try
            {
                ...
                if (combodisplay.Text != "")
                {
                    Models.stylegroupModel c1 = new Models.stylegroupModel();
                    c1.active = "";
                    Models.Validate va = new Models.Validate();
                    c1.asptblstygrpmasid = Convert.ToInt64("0" + combodisplay.Text);
                    c1.stylegroup = Convert.ToString(combodisplay.Text);
                    ...
                    else if (dt.Rows.Count != 0 && Convert.ToInt32("0" + combodisplay.Text) == 0 || Convert.ToInt32("0" + combodisplay.Text) == 0)
                    {
                    ...
                    else
                    {
                        ...
                        maxid = 0;
                        maxid = Convert.ToInt64(combodisplay.Text);
                    ...
                    if (cc >= 1)
                    {
                        for (i = 0; i < cc; i++)
                        {
                            ...
                            {
                                if (combodisplay.Text == "") { c.asptblstygrpmasid = Convert.ToInt64("0" + maxid); }
                                else { c.asptblstygrpmasid = Convert.ToInt64("0" + combodisplay.Text); }
                                c.stylename = Convert.ToInt64(dataGridView1.Rows[i].Cells[3].Value);
                                c.stylegroup = Convert.ToString(combodisplay.Text);
                    ...
                    if (combodisplay.Text == "")
                    {
                        MessageBox.Show("Record Saved Successfully " + combodisplay.Text, " Success Message ", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        News();

                    }
                    else
                    {
                        MessageBox.Show("Record Updated Successfully " + combodisplay.Text, " Update Message ", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        News();
        ...
        private void ListView1_ItemActivate(object sender, EventArgs e)
        {
            try
            {
                ...
                if (listView1.Items.Count > 0)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        combodisplay.Text = Convert.ToString(dt.Rows[0]["asptblstygrpmasid"].ToString());
                        combodisplay.Text = Convert.ToString(dt.Rows[0]["stylegroup"].ToString());
                        if (dt.Rows[0]["active"].ToString() == "T") { checkactive.Checked = true; } else { checkactive.Checked = false; }
                        string sel2 = "select a.asptblstygrpdetid, a.asptblstygrpmasid,b.asptblstymasid as stylename,a.stylegroup,a.notes from asptblstygrpdet a join asptblstymas b on b.asptblstymasid = a.stylename  where a.asptblstygrpmasid='" + txtstylegroupid.Text + "';";
                        ...
                        if (dt1.Rows.Count > 0)
                        {
                            ...
                            for (i = 0; i < dt1.Rows.Count; i++)
                            {
                                ...
                                if (Convert.ToInt64(dataGridView1.Rows[i].Cells[1].Value) > 0)
                                {
                                    ...
                                    dataGridView1.Rows[i].Cells[3].Value = Convert.ToInt64("0" + dt1.Rows[i]["stylename"].ToString());
            ...
            try
            {
                if (combodisplay.Text != "")
                {
                    ...
                    if (confirmation == DialogResult.Yes)
                    {
                        ...
                        string del1 = "delete from asptblstygrpmas where   asptblstygrpmasid='" + combodisplay.Text + "';";
                        Utility.ExecuteNonQuery(del1);
                        string del = "delete from asptblstygrpdet where  asptblstygrpdetid='" + combodisplay.Text + "';";
                        Utility.ExecuteNonQuery(del);
                        MessageBox.Show("Record Deleted Successfully " + combodisplay.Text, " Delete Message ", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void DisplayMaster_FormClosing(object sender, FormClosingEventArgs e)
        {
            _instance = null;
        }
    }
