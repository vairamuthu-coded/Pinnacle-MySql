...
namespace Pinnacle.Master.School.ChaitanyaSchool
{
    ...
    public partial class SectionMaster : Form,ToolStripAccess
    {
        ...
        public void Saves()
        {
            try
            {
                if (txtsection.Text != "")
                {
                    ...
                    //if (va.IsString(txtsection.Text) == true)
                    //{
        ...
        public void Deletes()
        {
            ...
            {
                string sel1 = "select a.asptblsectionmasID from asptblsectionmas a join asptblstudentmas b on a.asptblstudentmasid=b.section where a.asptblsectionmasID='" + txtsectionid.Text + "'";
                DataSet ds = Utility.ExecuteSelectQuery(sel1, "asptblsectionmas");
                DataTable dt = ds.Tables["asptblsectionmas"];
                if (dt.Rows.Count > 0)
                {
