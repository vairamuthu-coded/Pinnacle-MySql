...
namespace Pinnacle.Master
{
    public partial class SizeMaster : Form,ToolStripAccess
    {
        ...
        private void ListView1_ItemActivate(object sender, EventArgs e)
        {
            try
            {
                if (listView1.Items.Count > 0)
                {
                    ...
                    txtsizeid.Text = listView1.SelectedItems[0].SubItems[2].Text;
        ...
        public void Deletes()
        {
            if (txtsizeid.Text != "")
            {
                string sel1 = "select a.ASPTBLSIZMASID from ASPTBLSIZMAS a join asptblsizgrpdet b on a.ASPTBLSIZMASID=b.SIZENAME where a.ASPTBLSIZMASID='" + txtsizeid.Text + "'";
                DataSet ds = Utility.ExecuteSelectQuery(sel1, "ASPTBLSIZMAS");
                DataTable dt = ds.Tables["ASPTBLSIZMAS"];
                if (dt.Rows.Count > 0)
                {
