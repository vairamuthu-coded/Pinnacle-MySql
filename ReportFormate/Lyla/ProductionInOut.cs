...
{
    public partial class ProductionInOut : Form,ToolStripAccess
    {
        private static ProductionInOut _instance;
        public static ProductionInOut Instance
        {
            get
            {
                if (_instance == null)
                    _instance = new ProductionInOut();
                GlobalVariables.CurrentForm = _instance; return _instance;
            }
        }
        public ProductionInOut()
        {
            InitializeComponent();
        }
        ...
        private void empty()
        {
pictureBox1.Image = null; combotype.SelectedIndex = 0;
        ...
        {
            //   string sel = "SELECT A.asptblpurid,B.compcode,B.compname,B.address, a.barcode,A.PONO,A.colorname,A.sizename,'INWARD' AS INWARDDATA,b.companylogo FROM asptblpurdet1 A JOIN gtcompmast B ON A.compcode=B.gtcompmastid JOIN asptblpur C ON C.asptblpurid=A.asptblpurid AND C.compcode=B.GTCOMPMASTID  WHERE  a.stitching='F'   AND B.COMPCODE='" + Class.Users.HCompcode + "' AND A.pono='" + combopono.Text + "'  ORDER BY A.PONO,A.colorname, A.SIZENAME";
